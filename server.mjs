...
const razorpay = razorpayKeyId && razorpayKeySecret
  ...
  : null
const paymentProvider = razorpay ? "razorpay" : stripeSecretKey || mockStripeMode ? "stripe" : "not configured"
...
for (const statement of [
  "ALTER TABLE subscriptions ADD COLUMN payment_provider TEXT NOT NULL DEFAULT 'stripe'",
  "ALTER TABLE subscriptions ADD COLUMN razorpay_order_id TEXT",
  "ALTER TABLE subscriptions ADD COLUMN razorpay_payment_id TEXT",
  "ALTER TABLE subscriptions ADD COLUMN razorpay_signature TEXT",
]) {
  try { db.exec(statement) } catch (error) {
    if (!String(error.message).includes("duplicate column name")) throw error
  }
}
db.exec("CREATE UNIQUE INDEX IF NOT EXISTS subscriptions_razorpay_order_idx ON subscriptions(razorpay_order_id) WHERE razorpay_order_id IS NOT NULL")
...
function seedDatabase() {
  ...
  for (const article of [
    ...
    { id: "help_membership", slug: "membership-and-checkout", category: "Membership", title: "Plans, checkout, and status", excerpt: "Understand monthly and yearly plans and what happens after secure payment checkout.", body: "Choose the plan that fits your rhythm: monthly membership is flexible, while yearly membership covers twelve monthly entries. Checkout is handled by the configured payment provider. Your membership becomes active only after the signed payment verification updates the database, and the account page polls that status for confirmation.", tags: ["checkout", "plans", "billing"], popular: 1, sortOrder: 3 },
...
async function createRazorpayOrder({ name, email, plan, charity, memberId, subscriptionId }) {
  if (!razorpay) throw Object.assign(new Error("Razorpay is not configured on the server."), { statusCode: 503 })
  const selectedPlan = plans.get(plan)
  return razorpay.orders.create({
    amount: selectedPlan.amount * 100,
    currency: "INR",
    receipt: subscriptionId,
    notes: { member_id: memberId, plan, charity, email },
  })
}
...
function verifyRazorpaySignature(orderId, paymentId, signature) {
  ...
  return safeTextEqual(expected, signature)
}
...
function subscriptionResponse(memberId) {
  ...
  const subscription = db.prepare("SELECT s.id, s.plan, s.charity, s.currency, s.amount_minor AS amountMinor, s.status, s.payment_provider AS provider, s.stripe_session_id AS stripeSessionId, s.stripe_subscription_id AS stripeSubscriptionId, s.razorpay_order_id AS razorpayOrderId, s.razorpay_payment_id AS razorpayPaymentId, s.created_at AS createdAt, s.updated_at AS updatedAt FROM subscriptions s WHERE s.member_id = ? ORDER BY CASE s.status WHEN 'active' THEN 0 WHEN 'checkout_created' THEN 1 ELSE 2 END, s.updated_at DESC LIMIT 1").get(memberId) || null
...
app.post("/api/checkout/session", async (req, res) => {
  ...
  try {
    ...
    if (razorpay) {
      const order = await createRazorpayOrder({ name: String(name), email: String(email), plan, charity, memberId, subscriptionId })
      db.prepare(`INSERT INTO subscriptions (id, member_id, plan, charity, currency, amount_minor, status, payment_provider, razorpay_order_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(subscriptionId, memberId, plan, charity, "INR", selectedPlan.amount * 100, "checkout_created", "razorpay", order.id, timestamp, timestamp)
      return res.status(201).json({ provider: "razorpay", id: order.id, checkoutUrl: null, status: order.status, checkoutOptions: { key: razorpayKeyId, orderId: order.id, amount: order.amount, currency: order.currency, name: "Digital Heroes", description: selectedPlan.label, prefill: { name: String(name), email: String(email) }, notes: { plan, charity } }, plan, charity, currency: "INR", amount: selectedPlan.amount, amountMinor: selectedPlan.amount * 100 })
    }
    const stripeSession = await createStripeCheckout(req, { name: String(name), email: String(email), plan, charity, memberId })
    db.prepare(`INSERT INTO subscriptions (id, member_id, plan, charity, currency, amount_minor, status, payment_provider, stripe_session_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(subscriptionId, memberId, plan, charity, "INR", selectedPlan.amount * 100, "checkout_created", "stripe", stripeSession.id, timestamp, timestamp)
    return res.status(201).json({ provider: "stripe", id: stripeSession.id, checkoutUrl: stripeSession.url, status: stripeSession.status, plan, charity, currency: "INR", amount: selectedPlan.amount, amountMinor: selectedPlan.amount * 100 })
  } catch (error) {
...
app.post("/api/checkout/razorpay/verify", (req, res) => {
  ...
  if (!verifyRazorpaySignature(orderId, paymentId, signature)) return res.status(400).json({ message: "Razorpay payment verification failed." })
  const subscription = db.prepare("SELECT * FROM subscriptions WHERE payment_provider = 'razorpay' AND razorpay_order_id = ?").get(orderId)
  if (!subscription) return res.status(404).json({ message: "That Razorpay order could not be found." })
  const timestamp = now()
  db.prepare("UPDATE subscriptions SET status = 'active', razorpay_payment_id = ?, razorpay_signature = ?, updated_at = ? WHERE id = ?").run(paymentId, signature, timestamp, subscription.id)
  db.prepare("UPDATE members SET plan = ?, charity = ?, updated_at = ? WHERE id = ?").run(subscription.plan, subscription.charity, timestamp, subscription.member_id)
  return res.json({ ...subscriptionResponse(subscription.member_id), message: "Razorpay payment verified and membership activated." })
})
...
app.get("/api/checkout/session/status", async (req, res) => {
  ...
  const saved = db.prepare("SELECT id, plan, charity, currency, amount_minor AS amountMinor, status, payment_provider AS provider FROM subscriptions WHERE stripe_session_id = ? OR razorpay_order_id = ?").get(sessionId, sessionId)
  if (!saved) return res.status(404).json({ message: "That checkout session could not be found." })
  if (saved.provider === "stripe" && stripeSecretKey && saved.status === "checkout_created") {
    try {
  ...
  return res.json(db.prepare("SELECT id, plan, charity, currency, amount_minor AS amountMinor, status, payment_provider AS provider FROM subscriptions WHERE stripe_session_id = ? OR razorpay_order_id = ?").get(sessionId, sessionId))
})
...
app.get("/api/admin/subscriptions", requireRole("admin"), (_req, res) => {
  return res.json({ subscriptions: db.prepare("SELECT id, member_id AS memberId, plan, charity, currency, amount_minor AS amountMinor, status, payment_provider AS provider, stripe_session_id AS stripeSessionId, stripe_subscription_id AS stripeSubscriptionId, razorpay_order_id AS razorpayOrderId, razorpay_payment_id AS razorpayPaymentId, created_at AS createdAt, updated_at AS updatedAt FROM subscriptions ORDER BY created_at DESC").all() })
})
...
app.listen(port, "0.0.0.0", () => console.log(`Digital Heroes Express server listening on 0.0.0.0:${port}`))
ipaymentProvidermport express from "express"
import Razorpay from "razorpay"
import { createHmac, createHash, randomBytes, timingSafeEqual } from "node:crypto"
import { mkdirSync, existsSync } from "node:fs"
import { join } from "node:path"
