...
function ChatConsole({ articles, aiStatus }) {
  ...
  async function send(message = input) {
    ...
    try { const data = await readJson("/api/help/chat", { method: "POST", body: JSON.stringify({ message: content, history: messages.slice(-8) }) }); setMessages([...nextMessages, { role: "model", content: data.reply, source: data.source, model: data.model }]) } catch (error) { setNotice(error.message) } finally { setSending(false) }
  }
  return <div className="support-chat-console"><div className="support-chat-heading"><div><span className={`support-live-status ${geminiOnline ? "" : "support-live-fallback"}`}><span className="live-dot" /> {geminiOnline ? `Gemini online / ${aiStatus.gemini.model}` : "Knowledge base fallback"}</span><strong>Ask the signal.</strong></div><button aria-label="Clear chat" className="glass-icon-button" disabled={sending || messages.length === 1} onClick={() => { setMessages([starter]); setNotice("") }} type="button"><Sparkles size={16} /></button></div><div className="support-chat-messages" aria-live="polite">{messages.map((message, index) => <div className={`support-message support-message-${message.role}`} key={`${message.role}-${index}`}><span>{message.role === "model" ? <Bot size={14} /> : <MessageCircle size={14} />}</span><p>{message.content}</p>{message.source ? <small>{message.source === "gemini" ? `${message.model || aiStatus.gemini.model} / Gemini-powered` : "Help Center knowledge base"}</small> : null}</div>)}{sending ? <div className="support-message support-message-model"><span><Bot size={14} /></span><p className="support-thinking">Thinking<span>.</span><span>.</span><span>.</span></p></div> : null}</div>{suggestions.length ? <div className="support-suggestions">{suggestions.map((article) => <button className="glass-button" disabled={sending} key={article.slug} onClick={() => send(article.title)} type="button">{article.title}</button>)}</div> : null}<form className="support-chat-form" onSubmit={(event) => { event.preventDefault(); send() }}><input aria-label="Ask AI support" onChange={(event) => setInput(event.target.value)} placeholder="Ask about your membership…" value={input} /><button aria-label="Send support question" className="glass-submit-button" disabled={!input.trim() || sending} type="submit"><Send size={17} /></button></form>{notice ? <p className="support-chat-error" role="alert">{notice}</p> : null}</div>
}
...
export default function HelpCenterPage() {
  ...
  useEffect(() => { readJson("/api/help/status").then(setAiStatus).catch(() => {}); const interval = window.setInterval(() => { loadArticles(query, activeCategory); readJson("/api/help/status").then(setAiStatus).catch(() => {}) }, 20000); return () => window.clearInterval(interval) }, [query, activeCategory])
  function chooseCategory(category) { setActiveCategory((current) => current === category ? "" : category) }
  const geminiOnline = Boolean(aiStatus.gemini?.configured)
  return <div className="help-page"><header className="help-header"><div className="help-announcement"><span className="live-dot" /> Support signal / live knowledge base <span><Headphones size={13} /> {geminiOnline ? "Gemini online" : "Knowledge base fallback"}</span></div><nav className="help-nav container"><button className="wordmark" onClick={() => window.location.assign("/")} type="button"><span className="wordmark-dot" /> digital <strong>heroes</strong></button><div className="help-nav-links"><button onClick={() => window.location.assign("/past-draws")} type="button">Past draws</button><button onClick={() => window.location.assign("/subscription-scores")} type="button">Plans & scores</button><button className="nav-cta glass-button" onClick={() => { setChatOpen(true); document.getElementById("support-chat")?.scrollIntoView({ behavior: "smooth" }) }} type="button"><MessageCircle size={15} /> Ask support</button></div></nav></header><main className="help-main container"><section className="help-hero"><div className="help-hero-copy"><p className="eyebrow"><span className="eyebrow-line" /> The help center / 006</p><h1>Answers with<br /><em>a little lift.</em></h1><p>Find a clear path through membership, scores, draws, and impact. Search the live knowledge base or ask the AI support signal when you want a faster answer.</p><div className="help-hero-actions"><button className="glass-button glass-button-primary" onClick={() => document.getElementById("support-search")?.focus()} type="button"><Search size={16} /> Search the center</button><button className="glass-button" onClick={() => { setChatOpen(true); document.getElementById("support-chat")?.scrollIntoView({ behavior: "smooth" }) }} type="button"><Sparkles size={16} /> Ask Gemini support</button></div></div><div className="help-hero-world"><SupportCanvas activeCategory={activeCategory} categories={categories} onCategorySelect={chooseCategory} /><span className="help-canvas-hint">drag to orbit / click a category</span></div></section><section className="help-search-zone" id="support-search"><div className="help-search-bar"><Search size={18} /><input aria-label="Search help center" onChange={(event) => setQuery(event.target.value)} placeholder="Search scores, membership, draws, or impact" value={query} />{query ? <button aria-label="Clear search" className="glass-icon-button" onClick={() => setQuery("")} type="button"><X size={15} /></button> : null}</div><div className="help-category-row"><button className={`glass-button ${!activeCategory ? "glass-button-active" : ""}`} onClick={() => setActiveCategory("")} type="button">All signals</button>{categories.map((category) => <button className={`glass-button ${activeCategory === category ? "glass-button-active" : ""}`} key={category} onClick={() => chooseCategory(category)} type="button">{category}</button>)}</div></section>{error ? <div className="help-error"><CircleHelp size={17} /> <span>{error}</span><button className="glass-button" onClick={() => loadArticles()} type="button">Retry</button></div> : null}<section className="help-workspace"><div className="help-articles-column"><div className="help-section-heading"><div><p className="eyebrow"><span className="eyebrow-line" /> Public knowledge base</p><h2>Find your<br /><em>clear next step.</em></h2></div><span>{loading ? "Updating signal" : `${articles.length} public articles`}</span></div><div className="help-article-grid">{loading ? <div className="help-empty"><Sparkles size={23} /><strong>Reading the live knowledge base...</strong><span>Fresh answers are moving into place.</span></div> : articles.length ? articles.map((article) => <ArticleCard article={article} key={article.slug} onSelect={selectArticle} selected={selectedArticle?.slug === article.slug} />) : <div className="help-empty"><CircleHelp size={23} /><strong>No public article matched that signal.</strong><span>Try a broader question or clear the filter.</span></div>}</div>{selectedArticle ? <article className="help-article-detail" id="support-article-detail"><span className="support-article-category">{selectedArticle.category}</span><h3>{selectedArticle.title}</h3><p>{selectedArticle.body}</p><div className="help-tag-row">{selectedArticle.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div></article> : null}</div><div className="help-chat-column" id="support-chat">{chatOpen ? <ChatConsole aiStatus={aiStatus} articles={articles} /> : <button className="help-chat-trigger" onClick={() => setChatOpen(true)} type="button"><MessageCircle size={20} /> Open the support signal</button>}</div></section></main><footer className="help-footer container"><button className="wordmark" onClick={() => window.location.assign("/")} type="button"><span className="wordmark-dot" /> digital <strong>heroes</strong></button><span>Support answers are drawn from the published public ledger.</span><button className="glass-button" onClick={() => window.location.assign("/")} type="button"><ArrowLeft size={14} /> Main signal</button></footer></div>
}
