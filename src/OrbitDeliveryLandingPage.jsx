...
function WalkingCourier({ paused, onToggle }) {
  ...
  useFrame((state) => {
    ...
    const routeCycle = (state.clock.elapsedTime * .32) % 2
    const routeProgress = routeCycle <= 1 ? routeCycle : 2 - routeCycle
    const routeAngle = .12 + routeProgress * 1.12
    const stride = Math.sin(walk)
    const bob = Math.abs(Math.cos(walk))
    if (courier.current) {
      courier.current.position.x = Math.cos(routeAngle) * 1.47
      courier.current.position.y = Math.sin(routeAngle) * 1.47 + bob * .025
      courier.current.rotation.z = (routeAngle - .65) * .12 + stride * .035
    }
  ...
  return <group ref={courier} onClick={(event) => { event.stopPropagation(); onToggle?.() }} onPointerOut={() => setHovered(false)} onPointerOver={(event) => { event.stopPropagation(); setHovered(true) }} position={[1.43, .18, .08]} rotation={[0, -.45, .08]} scale={hovered ? 1.08 : 1}>
...
function DeliveryPlanet({ paused, onToggle }) {
  ...
  return <group ref={planet}>
    <mesh><sphereGeometry args={[1.48, 64, 64]} /><meshPhysicalMaterial color="#378fc9" emissive="#0b4f89" emissiveIntensity={.18} metalness={.05} roughness={.7} /></mesh>
    <mesh scale={1.035}><sphereGeometry args={[1.48, 32, 32]} /><meshBasicMaterial color="#c7f1ff" transparent opacity={.2} wireframe /></mesh>
    <mesh scale={1.075}><sphereGeometry args={[1.48, 32, 32]} /><meshBasicMaterial color="#90d7ef" transparent opacity={.1} wireframe /></mesh>
    {land.map((shape, index) => <mesh key={index} position={shape.slice(0, 3)} scale={shape.slice(3)}><sphereGeometry args={[1, 20, 12]} /><meshStandardMaterial color={index % 2 ? "#a8d77f" : "#dcefc5"} roughness={.8} /></mesh>)}
    ...
    <WalkingCourier onToggle={onToggle} paused={paused} />
  </group>
}

function OrbitStage({ paused, onToggle }) {
  return <div className="orbit-stage"><Canvas camera={{ position: [0, .1, 5.2], fov: 34 }} dpr={[1, 1.5]} gl={{ antialias: true, alpha: true }}><ambientLight intensity={1.5} /><directionalLight color="#ffffff" intensity={4} position={[3, 4, 5]} /><pointLight color="#6285ff" intensity={7} distance={8} position={[-3, -2, 3]} /><Sparkles count={90} scale={[7, 5, 3]} size={1.2} speed={.14} color="#6f91dc" opacity={.2} /><group position={[0, -.82, 0]} scale={1.22}><DeliveryPlanet onToggle={onToggle} paused={paused} /></group><OrbitControls enablePan={false} enableZoom={false} enableDamping dampingFactor={.08} rotateSpeed={.7} /></Canvas><div className="orbit-stage-topline"><span><span className="orbit-live-dot" /> Orbit delivery hero</span><span>drag to explore</span></div><button className="orbit-pause" onClick={onToggle} type="button">{paused ? <Play size={14} /> : <Pause size={14} />} {paused ? "Resume orbit" : "Pause to wave"}</button><span className="orbit-stage-label">drag in any direction<br />to rotate the world</span></div>
}
