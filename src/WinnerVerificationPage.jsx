...
function LoginPanel({ onLogin }) {
  ...
  async function submit(event) {
    ...
    try { const data = await api("/api/auth/login", { method: "POST", body: JSON.stringify({ email: form.get("email"), password: form.get("password") }) }); if (data.member.role !== "admin") throw new Error("Administrator access is required for the verification console."); onLogin(data.member) } catch (error) { setMessage(error.message); setStatus("error") }
  }
...
export default function WinnerVerificationPage() {
  ...
  useEffect(() => {
    api("/api/auth/session").then((data) => { if (data.member.role !== "admin") throw new Error("Administrator access is required."); setSession(data.member) }).catch((error) => { setNotice(error.message); setLoading(false) })
  }, [])
