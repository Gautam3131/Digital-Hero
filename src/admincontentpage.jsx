...
import { ArrowLeft, ArrowUpRight, Check, LogOut, Plus, Radio, Save, Send, ShieldCheck } from "lucide-react"
import { apiFetch } from "./apiBase"
import "./admin-content-page.css"

const emptyForm = { slug: "", category: "Membership", title: "", excerpt: "", body: "", tags: "", audience: "public" }
...
function ContentOrbit({ articles }) {
  ...
  const memberOnly = articles.filter((article) => article.audience === "members").length
  ...
  const nodes = [
    { position: [-1.45, 0.8, 0.2], color: "#d7fa6b" },
    { position: [1.4, 0.4, 0.25], color: "#ff715b" },
    { position: [0, -1.25, 0.35], color: "#74d5d8" },
  ]
...
function AdminLogin({ initialError = "", onLogin }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (!data.member || data.member.role !== "admin") throw new Error("Administrator access is required for the content studio.")
      onLogin(data.member)
    } catch (error) { setMessage(error.message || "Administrator sign-in could not be completed.") }
  }
  return <main className="content-login"><div className="content-login-card"><div className="content-brand"><span /> digital <strong>heroes</strong></div><p className="eyebrow"><span className="eyebrow-line" /> Content studio / 007</p><h1>Publish the<br /><em>next signal.</em></h1><p>Administrator access is required to edit the live knowledge base and publish updates.</p><form onSubmit={submit}><label>Email<input autoComplete="email" name="email" required type="email" placeholder="admin@example.com" /></label><label>Password<input autoComplete="current-password" name="password" required type="password" placeholder="Your password" /></label>{message ? <p className="content-error" role="alert">{message}</p> : null}<button className="button button-primary full-width" type="submit">Open content studio <ArrowUpRight size={16} /></button></form><button className="text-button" onClick={() => window.location.assign("/admin-dashboard")} type="button"><ArrowLeft size={15} /> Back to control room</button></div></main>
}
...
export default function AdminContentPage() {
  ...
  async function load() {
    ...
    if (selectedId) {
      ...
      if (selected) setForm({ ...selected, audience: selected.audience || "public", tags: selected.tags.join(", ") })
    }
  }

  useEffect(() => { api("/api/auth/session").then((data) => { if (!data.member || data.member.role !== "admin") throw new Error("Administrator access is required."); setSession(data.member); return load() }).catch((loadError) => { if (loadError.message !== "Sign in is required.") setError(loadError.message || "Administrator access could not be verified.") }) }, [])

  const counts = useMemo(() => ({ published: articles.filter((article) => article.active).length, drafts: articles.filter((article) => !article.active).length, memberOnly: articles.filter((article) => article.audience === "members").length }), [articles])
  function choose(article) { setSelectedId(article.id); setForm({ ...article, audience: article.audience || "public", tags: article.tags.join(", ") }); setNotice(""); setError("") }
