...
function AdminLogin({ initialError = "", onLogin }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (!data.member || data.member.role !== "admin") throw new Error("Administrator access is required for this surface.")
      onLogin(data.member)
    } catch (error) { setMessage(error.message || "Administrator sign-in could not be completed."); setStatus("error") }
  }
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => { api("/api/auth/session").then((data) => { if (!data.member || data.member.role !== "admin") throw new Error("Administrator access is required."); setSession(data.member); return refresh() }).catch((error) => { if (error.message !== "Sign in is required.") setAccessError(error.message || "Administrator access could not be verified.") }) }, [])
  useEffect(() => {
    if (!session) return undefined
    const interval = window.setInterval(() => refresh().catch(() => {}), 10000)
    return () => window.clearInterval(interval)
  }, [session])
  if (!session) return <AdminLogin initialError={accessError} onLogin={(member) => { setAccessError(""); setSession(member); refresh().catch((error) => setAccessError(error.message || "The control room could not load.")) }} />
